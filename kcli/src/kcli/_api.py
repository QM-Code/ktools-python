...
class InlineParser:
    ...
    def set_handler(self, option: str, handler: Callable[..., None], description: str) -> None:
    ...
    def set_optional_value_handler(
        self,
        option: str,
        handler: ValueHandler,
        description: str,
...
class Parser:
    ...
    def set_positional_handler(self, handler: PositionalHandler) -> None:
        set_positional_handler(self._data, handler)

    def add_inline_parser(self, parser: InlineParser) -> None:
        add_inline_parser(self._data, clone_inline_parser_data(parser._data))

    def parse_or_exit(self, argv: Sequence[str] | None) -> None:
        try:
            self.parse(argv)
        except CliError as exc:
            report_cli_error_and_exit(str(exc))

    def parse(self, argv: Sequence[str] | None) -> None:
        parse(self._data, argv)
